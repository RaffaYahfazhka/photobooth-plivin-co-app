...
const StripCustomizer = ({ customization, onChange }: Props) => {
  ...
  const hideBgTab = ["polaroid", "noir", "film", "valentine", "foodie", "hotel"].includes(customization.frameId);
      ...
      <div className="mb-3 flex gap-1 rounded-full bg-secondary p-1">
        {tabs.map((tab) => (
          <button
            ...
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-full py-2 text-xs font-semibold transition-all ${activeTab === tab.id
          ...
          {STRIP_FRAMES.map((f) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1.5 rounded-lg p-2 transition-all ${isActive
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {BG_COLORS.map((color) => (
            <button
              ...
              className={`flex flex-col items-center gap-1 rounded-lg p-2 transition-all ${customization.bgColor === color.value
                  ? "ring-2 ring-primary ring-offset-2 ring-offset-card"
                  : "hover:bg-secondary"
                }`}
            >
          ...
          {FILTERS.map((f) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1 rounded-lg p-1.5 transition-all ${isActive
                    ? "ring-2 ring-primary ring-offset-2 ring-offset-card"
                    : "hover:bg-secondary"
                  }`}
              >
